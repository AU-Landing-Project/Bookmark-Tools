<?php 
	...
	if(!empty($page_owner) && (($page_owner instanceof ElggUser) || ($page_owner instanceof ElggGroup))) {
		...
		// get data
		// build page elements
		$title_text = elgg_echo("bookmark_tools:new:title");
		...
		if(elgg_is_xhr()){
			...
			echo elgg_view_form("bookmark_tools/folder/edit", $form_vars, $body_vars);
			echo "</div>";
		} else {
			echo elgg_view_page($title_text, elgg_view_layout("one_sidebar", array(
				"title" => $title_text,
				"content" => elgg_view_form("bookmark_tools/folder/edit", $form_vars, $body_vars)
			)));
		}
	} else {
		forward();
