...
$parent_guid = (int)get_input("bookmark_tools_parent_guid", 0); // 0 is top_level
$access_id = get_input("access_id", ACCESS_DEFAULT);
$change_children_access = get_input("change_children_access", false);
$change_bookmarks_access = get_input("change_bookmarks_access", false);
...
if(!empty($title) && !empty($owner_guid))
{
	...
	if($owner && ($owner instanceof ElggUser || $owner instanceof ElggGroup))
	{
		...
		else
		{
			...
			$folder->subtype = BOOKMARK_TOOLS_SUBTYPE;
...
{
	register_error(elgg_echo("bookmark_tools:action:edit:error:input"));
}
