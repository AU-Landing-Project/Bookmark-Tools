<?php
	...
	if(!empty($folder)) {
		...
		$parent = get_input("bmfolder_guid", 0);
	...
	$form_data .= "<div>";
	$form_data .= "<label>" . elgg_echo("bookmark_tools:forms:edit:title") . "</label>";
	$form_data .= elgg_view("input/text", array("name" => "title", "value" => $title));
	$form_data .= "</div>";
	
	$form_data .= "<div>";
	$form_data .= "<label>" . elgg_echo("bookmark_tools:forms:edit:description") . "</label>";
	...
	$form_data .= "<br />";
	$form_data .= elgg_view("input/bmfolder_select", array("name" => "bookmark_tools_parent_guid", "bmfolder" => $folder, "value" => $parent, "container_guid" => $page_owner->getGUID(), 'type' => 'bmfolder'));
	$form_data .= "</div>";
	
	// set context to influence access
	$context = elgg_get_context();
	elgg_set_context("bookmark_tools");
