<?php 
	...
	if($folder_guids = $widget->folder_guids){
		...
		foreach($folder_guids as $guid){
			if(($folder = get_entity($guid)) && ($folder->getSubtype() == BOOKMARK_TOOLS_SUBTYPE)){
		...
		if(!empty($folders)){
			...
			if(elgg_instanceof($owner, "user")){
				$more_url = elgg_get_site_url() . "bookmarks/owner/" . $owner->username;
			} else {
				$more_url = elgg_get_site_url() . "bookmarks/group/" . $owner->getGUID() . "/all";
			}
			
			echo $folders;
			
			echo "<div class='widget_more_wrapper'>";
			echo elgg_view("output/url", array("href" => $more_url, "text" => elgg_echo("widgets:bookmark_tools:more")));
			echo "</div>";
		} else {
			echo elgg_echo("notfound");
		}
	} else {
		echo elgg_echo("widgets:bookmark_tools:no_folders");
	}
